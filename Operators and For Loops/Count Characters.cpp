...
That is, input will be a stream of characters and you need to consider all the characters which
are entered till '$'.
...
{
    char raj;
    int digit = 0, chr = 0, space = 0;
    while (raj != '$')
    {
        raj = cin.get();
        if ((raj >= 'a' && raj <= 'z') || (raj >= 'A' && raj <= 'Z'))
        {
            chr++;
        }
        else if (raj >= '0' && raj <= '9')

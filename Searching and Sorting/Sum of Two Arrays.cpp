...
// 1st Approach
#include <bits/stdc++.h>
void sumOfTwoArrays(int *arr1, int n, int *arr2, int m, int *out)
{
    int number = 0;
    int power = 1;
    for (int i = n - 1; i >= 0; i--)
    {
        number = (arr1[i] * power) + number;
        power *= 10;
    }
    int num = 0;
    int pow = 1;
    ...
    { // Here 0 in 1st position will not give error as per sample output2 thats why noty make case of digit may be +1
        out[i] = x % 10;
        x = x / 10;
    }
}
